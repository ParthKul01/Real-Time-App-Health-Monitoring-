 ...
 */
function MonitorCard({ id, title, target, status = 'up', latency = 0, uptime = 100, lastChecked, onDelete }) {
  const statusConfig = {
    up: {
      label: 'Operational',
      dot: 'bg-green-500',
    ...
    <div className="bg-white border border-[#B0BEC5] rounded-[24px] p-6 transition-all duration-300 shadow-md hover:shadow-xl hover:-translate-y-1">
      ...
      <div className="flex items-center justify-between mb-1">
        <div className="flex flex-col">
          ...
          <span className="text-[10px] text-[#90A4AE] font-bold uppercase tracking-tighter truncate max-w-[160px]">
            {target || 'Live Service'}
          </span>
        </div>
        <span className={`flex items-center gap-2 text-[10px] uppercase tracking-widest font-black px-3 py-1.5 rounded-full border shadow-sm ${cfg.badge}`}>
          <span className={`inline-block w-2 h-2 rounded-full ${cfg.dot} ${status === 'up' || status === 'pending' ? 'animate-pulse' : ''}`} />
          {cfg.label}
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 mt-5">
        ...
        <div className="bg-[#CFD8DC]/20 rounded-2xl p-4 border border-[#B0BEC5]/30">
          ...
          <div className="flex items-baseline gap-1">
            <span className="text-2xl font-black text-[#37474F]">{Number(uptime).toFixed(1)}</span>
        ...
        <div className="flex items-center gap-2">
          ...
          {onDelete && (
            <button
              ...
              className="text-[#90A4AE] hover:text-red-500 transition-colors text-xs font-black px-1"
            >
              ✕
            </button>
          )}
